...
pgfault(struct UTrapframe *utf)
{
	...
	if (!(utf->utf_err & FEC_WR) || !(uvpt[(uint32_t)addr / PGSIZE] & PTE_COW)) {
		panic("pgfault error");
	}
	...
	if ((r = sys_page_map(0, (void *)PFTEMP, 0, (void *)ROUNDDOWN(addr, PGSIZE), PTE_P|PTE_W|PTE_U)) < 0) {
...
fork(void)
{
	...
	uint32_t pd, pt;
	// UTOP is the UXSTACKTOP, so don't map the last page
	for (pd = 0; pd < PDX(UTOP); ++pd) {
		if (uvpd[pd] & PTE_P) {
			for (pt = 0; pt < NPTENTRIES; ++pt) {
				if ((uvpt[pd * NPTENTRIES + pt] & (PTE_P|PTE_U)) == (PTE_P|PTE_U)
						&& pd * NPTENTRIES + pt != PGNUM(UXSTACKTOP - PGSIZE)) {
					duppage(envid, pd * NPTENTRIES + pt);
				}
			}
		}
	}
