...
// the bootcpu's initial order is not sure
...
//		user_mem_assert(curenv, UXSTACKTOP - PGSIZE, PGSIZE, PTE_U | PTE_P | PTE_W);
		uintptr_t *ux_esp = (uintptr_t *)UXSTACKTOP;
		// if the page fault upcall recursively
		if (tf->tf_esp >= UXSTACKTOP - PGSIZE && tf->tf_esp < UXSTACKTOP) {
			ux_esp = (uintptr_t *)tf->tf_esp;
			ux_esp -= 1;
			*ux_esp = 0;
		}
		// in case the user exception stack overflow or didn't alloc the user exception stack
		user_mem_assert(curenv, (char *)ux_esp - sizeof(struct UTrapframe), sizeof(struct UTrapframe), PTE_P | PTE_U | PTE_W);
